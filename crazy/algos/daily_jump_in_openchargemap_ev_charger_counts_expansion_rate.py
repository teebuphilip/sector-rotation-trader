...
class DailyJumpInOpenchargemapEvChargerCountsExpansionRateAlgo(CrazyAlgoBase):
    algo_id = "daily-jump-in-openchargemap-ev-charger-counts-expansion-rate"
    name = "Daily Jump In Openchargemap Ev Charger Counts Expansion Rate Signals Bullish Consumer Discretionary"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.005 / 2.0)
            else:
                should_exit = should_exit or change > -(0.005 / 2.0)
            if should_exit:
