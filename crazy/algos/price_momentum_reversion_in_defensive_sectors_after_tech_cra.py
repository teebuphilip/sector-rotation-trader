...
class PriceMomentumReversionInDefensiveSectorsAfterTechCraAlgo(CrazyAlgoBase):
    algo_id = "price-momentum-reversion-in-defensive-sectors-after-tech-cra"
    name = "Price Momentum Reversion In Defensive Sectors After Tech Crash"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_prices(tickers=['XLK'], period="2y")
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.04 / 2.0)
            else:
                should_exit = should_exit or change > -(0.04 / 2.0)
            if should_exit:
