...
class DailyJumpInGoogleTrendsForHomeApplianceRepairSignalAlgo(CrazyAlgoBase):
    algo_id = "daily-jump-in-google-trends-for-home-appliance-repair-signal"
    name = "Daily Jump In Google Trends For Home Appliance Repair Signals Consumer Stress Lifting Consumer Staples"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='home appliance repair', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.12 / 2.0)
            else:
                should_exit = should_exit or change > -(0.12 / 2.0)
            if should_exit:
