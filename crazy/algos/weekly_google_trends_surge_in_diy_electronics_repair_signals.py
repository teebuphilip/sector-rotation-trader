...
class WeeklyGoogleTrendsSurgeInDiyElectronicsRepairSignalsAlgo(CrazyAlgoBase):
    algo_id = "weekly-google-trends-surge-in-diy-electronics-repair-signals"
    name = "Weekly Google Trends Surge In Diy Electronics Repair Signals Tech Consumer Stress Lift"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='DIY electronics repair', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.18 / 2.0)
            else:
                should_exit = should_exit or change > -(0.18 / 2.0)
            if should_exit:
