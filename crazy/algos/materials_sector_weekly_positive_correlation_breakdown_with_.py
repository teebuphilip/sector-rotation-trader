...
class MaterialsSectorWeeklyPositiveCorrelationBreakdownWithAlgo(CrazyAlgoBase):
    algo_id = "materials-sector-weekly-positive-correlation-breakdown-with-"
    name = "Materials Sector Weekly Positive Correlation Breakdown With Xlb"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_prices(tickers=['XLB'], period="2y")
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.06 / 2.0)
            else:
                should_exit = should_exit or change > -(0.06 / 2.0)
            if should_exit:
