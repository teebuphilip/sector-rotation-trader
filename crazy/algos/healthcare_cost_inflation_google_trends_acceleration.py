...
class HealthcareCostInflationGoogleTrendsAccelerationAlgo(CrazyAlgoBase):
    algo_id = "healthcare-cost-inflation-google-trends-acceleration"
    name = "Healthcare Cost Inflation Google Trends Acceleration"
    ...
    def compute_signal(self, as_of: date, state: dict, historical: bool = False):
        ...
        df = fetch_google_trends(keyword='healthcare costs rising', days_back=180)
        ...
        if held:
            ...
            if "up" == "up":
                should_exit = should_exit or change < (0.8 / 2.0)
            else:
                should_exit = should_exit or change > -(0.8 / 2.0)
            if should_exit:
