...
class HousingPermitVelocityAlgo(CrazyAlgoBase):
    algo_id = "housing-permit-velocity"
    name = "Housing Permit Velocity"
    ...
    def universe(self):
        return ["XLB", "XLI", "XLRE", "XLP", "XLU", "XLV"]
    ...
    def compute_signal(self, as_of, state, historical=False):
        ...
        meta["permits_3m_chg"] = round(chg, 4)

        if chg > self.BULL_3M:
            meta["signal_label"] = "HOUSING_EXPANSION"
            meta["signal_key"] = "HOUSING_EXPANSION"
            return "HOUSING_EXPANSION"
        if chg < self.BEAR_3M:
            meta["signal_label"] = "HOUSING_SLOWDOWN"
            meta["signal_key"] = "HOUSING_SLOWDOWN"
            return "HOUSING_SLOWDOWN"
    ...
    def target_allocations(self, signal, state, as_of):
        if signal == "HOUSING_EXPANSION":
            return {"XLB": 0.40, "XLI": 0.35, "XLRE": 0.25}
        if signal == "HOUSING_SLOWDOWN":
